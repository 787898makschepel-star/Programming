...
export const mcpOptions = {
    ...
    pageIdRouting: {
        type: 'boolean',
        describe: 'Require pageId on page-scoped tools and route requests by page ID (useful for concurrent agent sessions). Use --no-page-id-routing to disable.',
        default: true,
    },
    ...
    experimentalScreencastFps: {
        type: 'number',
        describe: 'Frames per second to use for screencast recording. Lower values can reduce memory pressure on pages that produce frames faster than ffmpeg can encode them.',
        implies: 'experimentalScreencast',
        coerce: (value) => {
            if (value === undefined) {
                return;
            ...
            if (!Number.isInteger(value) || value <= 0) {
                throw new Error(`Invalid experimentalScreencastFps ${value}. Expected a positive integer.`);
            }
            return value;
        },
    },
    ...
    javascriptEvaluation: {
        type: 'boolean',
        default: true,
        describe: 'Set to false to disable JavaScript execution. When disabled, evaluation tools (evaluate_script and slim evaluate) are disabled, the initScript parameter in navigate_page is turned off, and navigating to javascript:, data:, or vbscript: URLs is disallowed.',
    },
    sourceMaps: {
        type: 'boolean',
        default: true,
        describe: 'Whether to enable source maps in DevTools. Use --no-source-maps to disable.',
    },
    clearcutEndpoint: {
        type: 'string',
        hidden: true,
...
export function getMcpOptionsForViaCli() {
    if (!('default' in mcpOptions.headless)) {
        throw new Error('headless cli option unexpectedly does not have a default');
    }
    if (!('default' in mcpOptions.experimentalStructuredContent)) {
        throw new Error('experimentalStructuredContent cli option unexpectedly does not have a default');
    }
    ...
    return {
        ...
        isolated: {
            ...mcpOptions.isolated,
            description: 'If specified, creates a temporary user-data-dir that is automatically cleaned up after the browser is closed. Defaults to true unless userDataDir is provided.',
        },
    };
}
/**
...
export function parser(version, argv = process.argv, env = process.env) {
    ...
    const yargsInstance = yargs(hideBin(argv))
        ...
        const unknownArgs = Object.keys(args).filter(arg => !cliOptionsAllowedArgs.includes(arg));
