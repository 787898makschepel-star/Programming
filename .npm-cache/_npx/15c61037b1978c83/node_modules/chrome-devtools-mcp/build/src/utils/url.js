 ...
 */
export function isAllowedUrl(url, options) {
    let parsed;
    try {
        parsed = new URL(url);
    }
...
export function validateUrl(url, options) {
    ...
    try {
        ...
        throw new Error(`Invalid URL: "${url}". URLs must be valid according to the URL standard.`);
    }
    if (javascriptEvaluation === false &&
        DISALLOWED_PROTOCOLS.has(parsed.protocol)) {
        throw new Error(`Navigating to ${parsed.protocol} URLs is not allowed when JavaScript evaluation is disabled.`);
    }
    if (!isAllowedUrl(url, { categoryExtensions })) {
        if (parsed.protocol === 'chrome-extension:') {
            throw new Error(`Navigating to chrome-extension: URLs is not allowed without --categoryExtensions.`);
        }
