...
export class MCPConnectionStore {
    constructor(options) {
        ...
        this.createConnectionManager =
            options.createConnectionManager ??
                (() => new MCPConnectionManager(options.userConfig, options.logger, options.deviceId));
    ...
    view({ scope, owned = scope !== undefined } = {}) {
        ...
        const disconnect = async (connectionId) => {
            const entry = await peek(connectionId);
            if (!entry) {
                throw new MongoDBError(ErrorCodes.UnknownConnectionId, `Connection "${connectionId}" does not exist or has expired.`);
            }
    ...
    async dialPreconfigured(entry) {
        ...
        try {
            ...
            throw new MongoDBError(ErrorCodes.MisconfiguredConnectionString, "The configured connection string is not valid or the server is unreachable.");
        }
    }
    /**
