...
export class ConnectionEntry {
    ...
    getServiceProvider() {
        ...
        if (state.connectedAtlasCluster && (state.tag === "connecting" || state.tag === "disconnected")) {
            throw new MongoDBError(ErrorCodes.NotConnectedToMongoDB, `Connection "${this.connectionId}" is still being established to Atlas cluster "${state.connectedAtlasCluster.clusterName}", try again in a few seconds.`);
        }
 ...
 */
export function atlasClusterSlug(projectName, clusterName) {
...
export function buildEntryName(rawName) {
