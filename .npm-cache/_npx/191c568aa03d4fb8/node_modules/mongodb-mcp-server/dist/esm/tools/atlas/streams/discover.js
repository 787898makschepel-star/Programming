...
function toConnectionInspect(data) {
...
function toAccountDetailsSummary(data) {
    return {
...
function buildProcessorStructuredContent(proc, { includePipeline = false } = {}) {
...
export class StreamsDiscoverTool extends StreamsToolBase {
    constructor() {
        ...
        this.outputSchema = DiscoverOutputSchema.shape;
    }
    async execute({ projectId, action, workspaceName, resourceName, responseFormat, cloudProvider, region, limit, pageNum, }, context) {
        switch (action) {
            case "list-workspaces":
                return this.listWorkspaces(projectId, responseFormat, limit, pageNum, context);
            case "inspect-workspace":
                return this.inspectWorkspace(projectId, this.requireWorkspaceName(workspaceName), responseFormat, context);
            case "list-connections":
                return this.listConnections(projectId, this.requireWorkspaceName(workspaceName), responseFormat, limit, pageNum, context);
            case "inspect-connection":
                return this.inspectConnection(projectId, this.requireWorkspaceName(workspaceName), this.requireResourceName(resourceName, "connection"), context);
            case "list-processors":
                return this.listProcessors(projectId, this.requireWorkspaceName(workspaceName), responseFormat, limit, pageNum, context);
            case "inspect-processor":
                return this.inspectProcessor(projectId, this.requireWorkspaceName(workspaceName), this.requireResourceName(resourceName, "processor"), context);
            case "diagnose-processor":
                return this.diagnoseProcessor(projectId, this.requireWorkspaceName(workspaceName), this.requireResourceName(resourceName, "processor"), context);
            case "get-networking":
    ...
    async listWorkspaces(projectId, responseFormat, limit, pageNum, context) {
    ...
    async inspectWorkspace(projectId, workspaceName, responseFormat, context) {
    ...
    async listConnections(projectId, workspaceName, responseFormat, limit, pageNum, context) {
        ...
        return {
            content: formatUntrustedData(`Found ${data.results.length} connection(s) in the requested workspace:`, JSON.stringify(connections, null, 2)),
            structuredContent: { connections: conciseConnections },
        };
    }
    async inspectConnection(projectId, workspaceName, connectionName, context) {
    ...
    async listProcessors(projectId, workspaceName, responseFormat, limit, pageNum, context) {
        ...
        return {
            content: formatUntrustedData(`Found ${data.results.length} processor(s) in the requested workspace:`, JSON.stringify(processors, null, 2)),
            structuredContent: { processors: conciseProcessors },
        };
    }
    async inspectProcessor(projectId, workspaceName, processorName, context) {
    ...
    async diagnoseProcessor(projectId, workspaceName, processorName, context) {
        ...
        if (processorResult.status === "fulfilled" && processorResult.value) {
            ...
            if (proc.stats && Object.keys(proc.stats).length > 0 && structuredContent.stats) {
                ...
                if (inputCount > 0 && outputCount === 0 && dlqCount > 0) {
                    const healthWarning = `All ${dlqCount} input messages went to DLQ (0 successful outputs). ` +
            ...
            const processorError = processorResult.status === "rejected" ? String(processorResult.reason) : "No data returned";
    ...
    async getNetworking(projectId, cloudProvider, region, context) {
        ...
        const sections = [];
        const structuredContent = { privateLinks: [] };
