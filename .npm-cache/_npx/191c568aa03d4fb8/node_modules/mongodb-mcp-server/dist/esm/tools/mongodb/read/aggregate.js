...
import { AGG_COUNT_MAX_TIME_MS_CAP, ONE_MB, CURSOR_LIMITS_TO_LLM_TEXT, CURSOR_LIMIT_KEYS, } from "../../../helpers/constants.js";
...
export class AggregateTool extends MongoDBToolBase {
    constructor() {
        ...
        this.outputSchema = AggregateOutputSchema;
    }
    async execute({ connectionId, database, collection, pipeline, responseBytesLimit }, context) {
        ...
        try {
            ...
            if (this.config.indexCheck) {
                const [usesVectorSearchIndex, indexName] = await this.isVectorSearchIndexUsed({ isSearchSupported, provider }, {
                    database,
                    collection,
                    pipeline,
                });
            ...
            return {
                content: formatUntrustedData(successMessage, ...(documents.length > 0 ? [JSON.stringify(documents)] : [])),
                structuredContent: {
                    documents,
    ...
    assertOnlyUsesPermittedStages({ isSearchSupported }, pipeline) {
    ...
    async isVectorSearchIndexUsed({ isSearchSupported, provider }, { database, collection, pipeline, }) {
    ...
    generateMessage({ count, documents, appliedLimits, }) {
