...
export class ListAlertsTool extends AtlasToolBase {
    constructor() {
        super(...arguments);
        this.description = "List triggered alerts for a MongoDB Atlas project. These are alerts Atlas has raised, not the alert configurations that define them. Defaults to OPEN alerts; set status to TRACKING or CLOSED to see others.";
        this.argsShape = {
            ...ListAlertsArgs,
        };
        this.outputSchema = ListAlertsOutputSchema;
    }
    async execute({ projectId, status, limit, pageNum }, context) {
        ...
        return {
            content: formatUntrustedData(`Found ${alerts.length} alerts with status "${status}" in project ${projectId} ${data?.totalCount !== undefined && `(total: ${data.totalCount})`}`, JSON.stringify(alerts)),
            structuredContent: {
                projectId,
                status,
                alerts,
