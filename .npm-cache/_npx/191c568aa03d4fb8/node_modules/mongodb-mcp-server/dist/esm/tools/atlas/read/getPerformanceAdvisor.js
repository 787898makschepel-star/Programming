...
export class GetPerformanceAdvisorTool extends AtlasToolBase {
    constructor() {
        ...
        this.outputSchema = GetPerformanceAdvisorOutputSchema;
    }
    async execute({ projectId, clusterName, operations, since, namespaces }, context) {
        const [suggestedIndexesResult, dropIndexSuggestionsResult, slowQueryLogsResult, schemaSuggestionsResult] = await Promise.allSettled([
            ...
            operations.includes("slowQueryLogs")
                ? getSlowQueries(this.apiClient, projectId, clusterName, since ? new Date(since) : undefined, namespaces, context)
                : Promise.resolve(undefined),
        ...
        const structuredContent = {
            ...
            clusterName,
            ...(operations.includes("suggestedIndexes") && {
                suggestedIndexes: suggestedIndexesResult.status === "fulfilled"
                    ? (suggestedIndexesResult.value?.suggestedIndexes ?? [])
                    : [],
            }),
            ...(operations.includes("dropIndexSuggestions") && {
                dropIndexSuggestions: dropIndexSuggestionsResult.status === "fulfilled"
                    ? {
            ...
            ...(operations.includes("slowQueryLogs") && {
                slowQueryLogs: slowQueryLogsResult.status === "fulfilled" ? (slowQueryLogsResult.value?.slowQueryLogs ?? []) : [],
            }),
            ...(operations.includes("schemaSuggestions") && {
                schemaSuggestions: schemaSuggestionsResult.status === "fulfilled"
                    ? (schemaSuggestionsResult.value?.recommendations ?? [])
                    : [],
            }),
        };
        ...
        const hasAnyRecommendations = (operations.includes("suggestedIndexes") && hasSuggestedIndexes) ||
