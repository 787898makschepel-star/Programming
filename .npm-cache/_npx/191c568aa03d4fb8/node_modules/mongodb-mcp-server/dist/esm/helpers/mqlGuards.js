 ...
 */
export function assertNoServerSideJS(value) {
    const operator = findServerSideJSOperator(value);
    if (operator) {
        throw new MongoDBError(ErrorCodes.ForbiddenServerSideJS, `The use of server-side JavaScript operators (such as ${SERVER_SIDE_JS_OPERATORS.join(", ")}) is disabled. The "${operator}" operator is not allowed. To enable it, set the "disableServerSideJs" configuration option to false.`);
    }
}
/**
 ...
 */
export function getWriteStageTargets(pipeline, defaultDatabase) {
    const targets = [];
