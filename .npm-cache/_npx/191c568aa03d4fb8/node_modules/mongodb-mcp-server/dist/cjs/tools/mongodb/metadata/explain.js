...
const verbosityEnum = zod_1.z.enum(["queryPlanner", "queryPlannerExtended", "executionStats", "allPlansExecution"]);
const ExplainOutputSchema = {
    explainResult: zod_1.z.record(zod_1.z.string(), zod_1.z.unknown()),
    method: zod_1.z.enum(["aggregate", "find", "count"]),
...
class ExplainTool extends mongodbTool_js_1.MongoDBToolBase {
    ...
    async execute({ connectionId, database, collection, method: methods, verbosity }, { signal }) {
        ...
        return {
            content: (0, tool_js_1.formatUntrustedData)(`Here is some information about the winning plan chosen by the query optimizer for running the given \`${method.name}\` operation on the requested namespace. The execution plan was run with the following verbosity: "${verbosity}". This information can be used to understand how the query was executed and to optimize the query performance.`, JSON.stringify({ database, collection, plan: result })),
            structuredContent: {
