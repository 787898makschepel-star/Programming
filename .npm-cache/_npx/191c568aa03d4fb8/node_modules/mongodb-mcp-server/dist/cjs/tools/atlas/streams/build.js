...
class StreamsBuildTool extends streamsToolBase_js_1.StreamsToolBase {
    ...
    fromValidationError(error) {
        return {
            content: StreamsBuildTool.textContentFrom(error),
            isError: error.isError ?? true,
        };
    }
    static textContentFrom(error) {
        return error.content.flatMap((block) => block.type === "text" ? [{ type: "text", text: block.text }] : []);
    }
    ...
    async createWorkspace(args, context) {
    ...
    async createConnection(args, context) {
        const workspaceName = this.requireWorkspaceName(args);
     ...
     */
    async normalizeAndValidateConnectionConfig(config, connectionType, context) {
    ...
    async validateKafkaConfig(config, context) {
    ...
    async validateClusterConfig(config, context) {
        const missingFields = StreamsBuildTool.collectMissingFields([
    ...
    async validateAwsConfig(config, connectionType, context) {
        ...
        return this.elicitOrReportMissing(connectionType, config, missingFields, context, (fields, cfg) => {
            if (fields.roleArn) {
                if (!cfg.aws)
                    cfg.aws = {};
    ...
    async validateSchemaRegistryConfig(config, context) {
    ...
    async validateHttpsConfig(config, context) {
        const missingFields = StreamsBuildTool.collectMissingFields([
     ...
     */
    async elicitOrReportMissing(connectionType, config, missingFields, context, applyFields, additionalNote) {
    ...
    async validatePipelineConnections(projectId, workspaceName, pipeline, context, dlq) {
    ...
    async createProcessor(args, context) {
        const workspaceName = this.requireWorkspaceName(args);
        ...
        if (structureError)
            return this.fromValidationError(structureError);
        const connectionError = await this.validatePipelineConnections(args.projectId, workspaceName, args.pipeline, context, args.dlq);
        if (connectionError)
            return this.fromValidationError(connectionError);
        const body = {
    ...
    async createPrivateLink(args, context) {
