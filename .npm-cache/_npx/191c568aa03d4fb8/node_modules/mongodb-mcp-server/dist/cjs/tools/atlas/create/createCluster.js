...
const encryptionAtRestProviderEnum = zod_1.z.enum(["AWS", "AZURE", "GCP", "NONE"]);
...
const ELECTABLE_NODE_DISTRIBUTIONS = [[3], [2, 1], [2, 2, 1]];
function buildReplicationSpecs(provider, regions, instanceSize, autoScaling, diskSizeGB) {
...
exports.CreateClusterArgsShape = {
    ...
    regions: zod_1.z
        ...
        .min(1)
        .max(3)
        .describe("Cloud provider regions in Atlas format using uppercase letters and underscores (e.g. US_EAST_1). The first region has the highest priority. Do not include duplicate regions."),
    ...
    encryptionAtRestProvider: encryptionAtRestProviderEnum
        .optional()
        .describe("Customer-managed key provider for encryption at rest. Defaults to the cluster's provider if a valid configuration exists. Use `NONE` to explicitly disable the feature. Omit unless explicitly specified by the user."),
};
...
class CreateClusterTool extends atlasTool_js_1.AtlasToolBase {
    constructor() {
        super(...arguments);
        this.description = "Create a MongoDB Atlas cluster (M10–M80, replica set or single shard, single or multi-region). " +
            "Compute autoscaling is enabled by default: min instance size is set to the selected instance size, max is set two tiers above. " +
    ...
    async execute(args, context) {
        ...
        const ipAccessListResult = await (0, accessListUtils_js_1.ensureCurrentIpInAccessList)(this.apiClient, projectId, context);
        const result = await this.apiClient.createCluster({
            params: { path: { groupId: projectId } },
            body,
        }, context);
        const ipAccessListNote = (0, accessListUtils_js_1.getAccessListNote)(ipAccessListResult);
        return {
            content: [
                {
                    type: "text",
                    text: `Cluster "${clusterName}" is being created in project "${projectId}" (${instanceSize} ${clusterType} on ${provider}/${regions.join(", ")}). ` +
    ...
    async doesValidEARConfigExist(provider, projectId, context) {
        try {
            const encryptionAtRest = await this.apiClient.getEncryptionAtRest({ params: { path: { groupId: projectId } } }, context);
            let config;
            switch (provider) {
                case "AWS":
            ...
            if (error instanceof apiClientError_js_1.ApiClientError && error.response.status === 403) {
                return false;
            }
            throw error;
        }
