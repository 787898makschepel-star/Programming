...
`),
        ...
        this.atlasSearchIndexDefinition = zod_1.z
            ...
            analyzer: zod_1.z
                ...
                .describe("The analyzer to use for the index. Can be one of the built-in lucene analyzers (`lucene.standard`, `lucene.simple`, `lucene.whitespace`, `lucene.keyword`), a language-specific analyzer, such as `lucene.cjk` or `lucene.czech`, or a custom analyzer defined by name in `analyzers`."),
            mappings: zod_1.z
                .object({
                dynamic: zod_1.z
                    .union([
                ...
                .describe("Document describing the index to create. `fields` is required and must define at least one field when `dynamic` is `false` (the default). When `dynamic` is `true` or a typeSet object, `fields` may still be provided to index additional fields explicitly."),
...
`);
    ...
    async execute({ connectionId, database, collection, name, definition: definitions, }) {
        ...
        switch (definition.type) {
            ...
            case "search":
                {
                    await this.assertSearchSupported(connectionId);
                    const searchIndexDefinition = { ...definition };
