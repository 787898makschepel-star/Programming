...
exports.assertNoServerSideJS = assertNoServerSideJS;
exports.isWriteStage = isWriteStage;
exports.getWriteStageTargets = getWriteStageTargets;
 ...
 */
const SERVER_SIDE_JS_OPERATORS = ["$where", "$function", "$accumulator"];
/**
 ...
 */
function assertNoServerSideJS(value) {
    const operator = findServerSideJSOperator(value);
    if (operator) {
        throw new errors_js_1.MongoDBError(errors_js_1.ErrorCodes.ForbiddenServerSideJS, `The use of server-side JavaScript operators (such as ${SERVER_SIDE_JS_OPERATORS.join(", ")}) is disabled. The "${operator}" operator is not allowed. To enable it, set the "disableServerSideJs" configuration option to false.`);
    }
}
/**
 ...
 */
function writeStageOperator(stage) {
    if ("$out" in stage) {
        return "$out";
    }
    if ("$merge" in stage) {
 ...
 */
function getWriteStageTargets(pipeline, defaultDatabase) {
    const targets = [];
 ...
 */
function resolveNamespace(target, defaultDatabase) {
...
//# sourceMappingURL=mqlGuards.js.map