...
const ServerConfigSchema = zod_1.z.object({
    ...
    loggers: zod_1.z
        ...
        .check(zod_1.z.refine((val) => new Set(val).size === val.length, {
    ...
    confirmationRequiredTools: zod_1.z
        ...
        .register(exports.configRegistry, { overrideBehavior: "merge" }),
    elicitationTimeoutMs: zod_1.z.coerce
        .number()
        .default(300000)
        .describe("Time in milliseconds the user has to respond to an elicitation request (such as a tool confirmation prompt) before it fails.")
    ...
    disableServerSideJs: zod_1.z
        .preprocess(configUtils_js_1.parseBoolean, zod_1.z.boolean())
        .default(true)
        .describe("When set to true, disallows the use of server-side JavaScript operators (such as $where, $function, and $accumulator) in query filters and aggregation pipelines.")
        .register(exports.configRegistry, {
        overrideBehavior: (0, configUtils_js_1.oneWayOverride)(true),
    }),
    telemetry: zod_1.z
    ...
    notificationTimeoutMs: zod_1.z.coerce
        ...
        .register(exports.configRegistry, { overrideBehavior: (0, configUtils_js_1.onlyLowerThanBaseValueOverride)() }),
    maxSessions: zod_1.z.coerce
        .number()
        .int()
        .min(1, "Invalid maxSessions: must be at least 1")
        .default(constants_js_1.DEFAULT_MAX_SESSIONS)
        .describe("Maximum number of concurrent sessions the HTTP transport will hold in memory (only used when transport is 'http'). Each session holds a full server instance, transport, and timers, so choose a value based on your deployment's available memory; the default is a conservative safety net rather than a recommended production value.")
        .register(exports.configRegistry, { overrideBehavior: "not-allowed" }),
    maxActiveConnections: zod_1.z.coerce
        .number()
        .int()
        .min(1, "Invalid maxActiveConnections: must be at least 1")
        .default(10)
        .describe("Maximum number of MongoDB connections a single scope (an MCP session by default, see connectionScope) can hold open. When exceeded, the scope's least-recently-used connection is closed and its connectionId revoked. The preconfigured connection does not count towards the limit.")
        .register(exports.configRegistry, { overrideBehavior: "not-allowed" }),
    connectionScope: zod_1.z
        .enum(["session", "global"])
        .default("session")
        .describe("Visibility scope for MongoDB connections created at runtime. With 'session' (the default), each MCP session only sees the connections it created (plus the shared 'preconfigured' one) and they are closed when the session ends — recommended when the HTTP transport is exposed to multiple clients without authentication. With 'global', connections are shared across all sessions and survive session rotation.")
        .register(exports.configRegistry, { overrideBehavior: "not-allowed" }),
