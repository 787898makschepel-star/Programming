...
exports.connectCapableTools = connectCapableTools;
const errors_js_1 = require("./errors.js");
const disconnect_js_1 = require("../tools/mongodb/connect/disconnect.js");
const listConnections_js_1 = require("../tools/mongodb/connect/listConnections.js");
/**
 ...
 */
function connectCapableTools(tools) {
    return tools
        .filter((t) => t.operationType === "connect" && t.name !== disconnect_js_1.DisconnectTool.toolName && t.isEnabled())
        .sort((a, b) => a.category.localeCompare(b.category));
}
const connectionErrorHandler = (error, { availableTools, connectionState }) => {
    ...
    switch (error.code) {
        case errors_js_1.ErrorCodes.UnknownConnectionId: {
            const listConnectionsAvailable = availableTools.some((t) => t.name === listConnections_js_1.ListConnectionsTool.toolName && t.isEnabled());
            return {
                errorHandled: true,
                result: {
                    content: [
                        {
                            type: "text",
                            text: `${error.message} ${listConnectionsAvailable
                                ? `Call the "${listConnections_js_1.ListConnectionsTool.toolName}" tool to see the active connections, or establish a new one and retry with the connectionId it returns.`
