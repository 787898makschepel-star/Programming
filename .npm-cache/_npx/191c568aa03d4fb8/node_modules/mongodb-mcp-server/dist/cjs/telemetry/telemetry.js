...
class Telemetry {
    constructor(config) {
        ...
        this.eventCache = config.eventCache ?? new eventCache_js_1.EventCache();
        this.deviceId = config.deviceId;
        this.pipelineCommonProperties = {
            ...constants_js_1.MACHINE_METADATA,
        };
    }
    static create(sessionOrConfig, userConfig, deviceId, { commonProperties = {}, eventCache, } = {}) {
    ...
    emitEvents(events) {
        ...
        this.whenReady()
            ...
            this.eventCache.appendEvents(events.map((event) => ({
                ...event,
     ...
     */
    whenReady() {
        return (this.setupPromise ?? Promise.resolve()).then(() => undefined, () => undefined);
    }
    /**
    ...
    async sendEvents(client, events, signal) {
        try {
            ...
            await client.sendEvents(events.map((event) => ({ ...event, properties: (0, mongodb_redact_1.redact)(event.properties, secrets) })), { signal: effectiveSignal });
